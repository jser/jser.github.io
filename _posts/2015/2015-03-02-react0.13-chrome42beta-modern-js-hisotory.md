...
Web Audio関連では[OfflineAudioContext](https://developer.mozilla.org/en-US/docs/Web/API/OfflineAudioContext "OfflineAudioContext")が最新の仕様に追従してPromiseを返すようになっています。

Chrome 41ではAudioContextで`suspend`と`resume`のサポート、StereoPannerNodeの追加など色々増えているので、併せてChromium 41ベースのOperaのリリースノートも見ておくといい気がします。
