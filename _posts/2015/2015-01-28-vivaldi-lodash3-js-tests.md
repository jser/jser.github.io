...
また`npm instal lodash.foreach`などのように個々のモジュールとして入れることは以前から可能でしたが、この時の依存関係ツリーがシンプルになるように改善されました。

またパフォーマンス面でもlazy chainingの導入やES6のSet & WeakMapを利用することで大きく改善されたようです。
