...
esbuild + Rollupの二重バンドラー構成をRustベースの統合バンドラーであるRolldownに置き換えました。lightningcssがpeerDependencyから直接的な依存関係へ変更され、ビルトインのDevtools、TypeScriptのパスエイリアス解決(`resolve.tsconfigPaths`)のサポートが追加されています。WebAssemblyのSSRサポート、ブラウザのコンソールログをdevサーバーのターミナルに転送する`server.forwardConsole`の追加なども含まれています。同じタイミングでVitest 4.1もリリースされており、Vite 8のサポート、Test Tags機能の追加、`aroundEach`/`aroundAll`フックの追加なども含まれています。
