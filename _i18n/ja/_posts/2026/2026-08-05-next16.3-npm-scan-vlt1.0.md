...
記事では、npmのpublish時のマルウェアスキャンとdual-use content向けメタデータについて書かれています。
新しくpublishされたパッケージは、インストール可能となる前にスキャンされる。その結果に応じて通常どおり公開、手動レビュー、ブロックのいずれかとなります。
また、マルウェアと誤認される可能性があるパッケージは`package.json`の`contentPolicy`と`DISCLOSURE`ファイルでdual-use contentを宣言する仕組みを追加されています。
宣言したパッケージは2FAを強制するtrusted publishing、対話的なpublish、staged publishingで公開する必要があリます。
...
依存パッケージのライフサイクルスクリプトを`vlt install`と`vlt build`に分けて扱うPhased Installations、npmの公開レジストリへのOIDC Trusted Publishingに対応しています。
Hosted Package Registriesはnpm Registry APIと互換性があり、npm、pnpm、Yarn、Bun、Denoから利用でき、既知のマルウェアをブロックする仕組みを持っています。
...
Hosted Package Registriesはnpm Registry APIと互換性があり、npm、pnpm、Yarn、Bun、Denoから利用でき、既知のマルウェアをブロックする仕組みを持っている。
