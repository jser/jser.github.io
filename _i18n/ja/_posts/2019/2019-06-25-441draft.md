...
`eslint:recommended`の設定から次のルールが追加/更新されています。
...
また、`eslint:recommended`の設定から次のルールがとりのぞかれて
...
テンプレートの構文などはそのままで、コンポーネントのライフサイクルに関する部分をReact Hooksのような関数ベースにかけるようにするProposalです。

すでにVue 2.xでもRFCの動作を再現したプラグインが公開されていますが、Proposalでも2.xで動作するプラグインを提供する予定とのことです。
