...
大きな変更として、`react`と`react-dom`のパッケージにモジュールが分けられています。

`ReactDOM.render`、`ReactDOM.unmountComponentAtNode`、`ReactDOM.findDOMNode`は`react`から`react-dom`に移され、
 `ReactDOMServer.renderToString`、 `ReactDOMServer.renderToStaticMarkup`は`react-dom/server`に移されています。
...
![ES6 code](http://monosnap.com/image/xvcrecVValrE4kIhOYHJw2KbeKLlMd.png)
 ...
 プロダクトとかライブラリをES6で書いたことがあるかという質問になると一気に減りそうな気がしますが、[Babel](http://babeljs.io/ "Babel")とかの変換ツールが気軽に使える様になったのが結構大きそうな気がします。
...
もう少しするとブラウザにも普通の開発で書くようなES6の機能は大体実装されるので(module以外...)、学ぶのネイティブで使うのはTranspiler経由というような使い分けができるようになると、もっとES6が身近になると思います。
