...
元々はLernaがWorkspace管理(依存関係の管理)、タスク管理(パッケージの依存を見てタスクを実装する機能)、Publish管理(バージョンの更新や公開)などの機能を持ったツールとしてスタートしています。
...
Publish管理は、まだ成熟したツールは少ないですが[changesets](https://github.com/changesets/changesets)や[lerna-lite](https://github.com/ghiscoding/lerna-lite)などが出てきています。
