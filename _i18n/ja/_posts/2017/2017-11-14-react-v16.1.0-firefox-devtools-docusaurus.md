...
Bower向けの配布は終了したため代わりに[CDN](https://reactjs.org/docs/installation.html#using-a-cdn)の利用の推奨しています。`React.Children` utilがPortalをサポート、React DOM Serverに`suppressHydrationWarning`属性の追加などが行われいます。
