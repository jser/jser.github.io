...
脆弱性があったため無効化されていたV8 Snapshotを再有効化されました。
...
Node.js 8.5.0で追加された`fs.copyFile`/`fs.copyFileSync`はOSネイティブのコマンドを使い、またファイルシステムによってはcopy-on-writeとなるため大量のファイルをコピーに向いています。
...
Yarnでは利用できる環境では`fs.copyFile`を使うことでインストール速度改善されているようです。
