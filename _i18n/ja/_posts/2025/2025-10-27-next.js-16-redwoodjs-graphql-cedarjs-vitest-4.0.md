---
title: "2025-10-27のJS: Next.js 16、RedwoodJS GraphQL → CedarJS、Vitest 4.0"
author: "azu"
layout: post
...
Node.js 18のサポート終了、AMP、`next lint`、`middleware.ts`の削除など破壊的な変更が含まれています。
新しい機能として、Cache Componentsによる明示的なキャッシュ制御とPPR統合、Next.js Devtools MCPの追加、`middleware.ts`を`proxy.ts`に変更などが含まれています。
また、Turbopackがデフォルトバンドラとなり、React Compiler統合、ルーティング・プリフェッチの最適化、React 19.2のサポートなども追加されています。

---

RedwoodJSの開発が終了し、CedarJSへの移行が推奨されています。
...
CedarJSはRedwoodJSをforkしたReact + GraphQLのWebアプリケーションフレームワークで、今後はこちらで開発が継続されます。

また、Redwood自体は[RedwoodSDK](https://github.com/redwoodjs/sdk)というCloudflare Workers向けのReactフレームワークを開発に集中しています。

---

Vitest 4.0がリリースされました。

- [Vitest 4.0 is out! | Vitest](https://vitest.dev/blog/vitest-4)

Browser Modeがstableとなり、ビジュアルリグレッションテストをする`toMatchScreenshot`のサポート、Playwright TraceとframeLocatorの追加が含まれています。また、`expect.assert`と`expect.schemaMatching`が追加されました。破壊的な変更として、`basic`レポーターの削除、Browser Provider設定の変更、`workspace`オプションの削除、`coverage.all`と`coverage.extensions`の削除などが含まれています。
...
Redwood GraphQL(RedwoodJS) 8.9.0リリース。
RedwoodJSは開発が終了したため、CedarJSへの移行が推奨されている

- [CedarJS | The React + GraphQL Web App Framework](https://cedarjs.com/ "CedarJS | The React + GraphQL Web App Framework")

Redwood自体は[RedwoodSDK](https://github.com/redwoodjs/sdk)というCloudflare Workers向けのReactフレームワークを開発している。
