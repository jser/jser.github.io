...
DotenvPluginとWebpackManifestPluginが追加されました。また、`import.meta.env`、`import.meta.dirname`、`import.meta.filename`のサポートが含まれています。その他、`import.defer()`のサポート、CSS向けの`exportType`オプションの追加、CSS Modulesの`composes`プロパティのサポートなどが含まれています。
