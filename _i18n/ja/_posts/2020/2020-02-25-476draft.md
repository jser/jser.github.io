...
また、`tsconfig.json`の`tartet`と`module`オプションに`es2020`を追加、`watchOptions`で`--watch`時の挙動の設定の追加、`assumeChangesOnlyAffectDirectDependencies`オプションで"Fast and Loose"なインクリメンタルビルドをサポートされています。
