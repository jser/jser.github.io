...
`<tempalte>`要素の`showroot`属性で宣言的にShadow Root宣言できるようになる[Proposal](https://github.com/mfreed7/declarative-shadow-dom)です。
...
Declarative Shadow DOMの主な目的であるSSRの対応、`getInnerHTML()`でのShaowRootの取得、Custom Elementの`attachShadow()`の挙動、細かい挙動の解説がされています。
