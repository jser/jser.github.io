...
JSer.info #511 - Reat 17が正式リリースされました。
...
その他の破壊的な変更としては、event poolingの仕組みの削除と`event.persist()`の無効化、`onScroll`がバブリングしなくなることや`useEffect`のcleanup処理が必ず非同期となると言った変更も含まれています。
