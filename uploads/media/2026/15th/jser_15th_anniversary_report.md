...
| 種類 | 2011-2013 | 2014-2016 | 2017-2019 | 2020-2022 | 2023-2025 | 変化 |
|------|-----------|-----------|-----------|-----------|-----------|------|
| リリースノート | 560 | 750 | 862 | 937 | 986 | +76% |
| 解説記事 | 719 | 692 | 813 | 617 | 526 | -27% |
| ライブラリ/ツール | 371 | 599 | 446 | 397 | 191 | -49% |
| スライド/動画 | 292 | 309 | 170 | 60 | 49 | -83% |
| 書籍関連 | 192 | 203 | 135 | 73 | 45 | -77% |
| チュートリアル | 97 | 154 | 93 | 33 | 14 | -86% |

### 変化のポイント

- リリースノート: 560件 → 986件（76%増加）
- 書籍関連: 192件 → 45件（77%減少）
- スライド/動画: 292件 → 49件（83%減少）
- チュートリアル: 97件 → 14件（86%減少）
...
SELECT
    ...
    CASE
        -- タグ優先
        WHEN list_contains(tags, 'ReleaseNote') OR list_contains(tags, 'news') THEN 'リリースノート'
        WHEN list_contains(tags, 'book') OR list_contains(tags, 'ebook') THEN '書籍関連'
        WHEN list_contains(tags, 'slide') OR list_contains(tags, 'video') THEN 'スライド/動画'
        WHEN list_contains(tags, 'tutorial') THEN 'チュートリアル'
        WHEN list_contains(tags, 'article') THEN '解説記事'
        WHEN list_contains(tags, 'library') OR list_contains(tags, 'software') THEN 'ライブラリ/ツール'
        -- キーワードフォールバック
        WHEN LOWER(content) LIKE '%リリース%' OR LOWER(content) LIKE '%release%' THEN 'リリースノート'
        WHEN LOWER(content) LIKE '%書籍%' OR LOWER(content) LIKE '%書評%' THEN '書籍関連'
        WHEN LOWER(content) LIKE '%スライド%' THEN 'スライド/動画'
        WHEN LOWER(content) LIKE '%入門%' OR LOWER(content) LIKE '%tutorial%' OR LOWER(content) LIKE '%チュートリアル%' THEN 'チュートリアル'
        WHEN LOWER(content) LIKE '%解説%' OR LOWER(content) LIKE '%について%' THEN '解説記事'
        WHEN LOWER(content) LIKE '%ライブラリ%' OR LOWER(content) LIKE '%library%' THEN 'ライブラリ/ツール'
        ELSE 'その他'
    END as article_type,
    COUNT(*) as count
FROM read_json_auto('https://jser.info/source-data/items.json', ignore_errors=true)
...
├───────────┼───────────────────┼───────┤
│ 2011-2013 │ その他            │  1079 │
│ 2011-2013 │ スライド/動画     │   292 │
│ 2011-2013 │ チュートリアル    │    97 │
│ 2011-2013 │ ライブラリ/ツール │   371 │
│ 2011-2013 │ リリースノート    │   560 │
│ 2011-2013 │ 書籍関連          │   192 │
│ 2011-2013 │ 解説記事          │   719 │
│ 2014-2016 │ その他            │   561 │
│ 2014-2016 │ スライド/動画     │   309 │
│ 2014-2016 │ チュートリアル    │   154 │
│ 2014-2016 │ ライブラリ/ツール │   599 │
│ 2014-2016 │ リリースノート    │   750 │
│ 2014-2016 │ 書籍関連          │   203 │
│ 2014-2016 │ 解説記事          │   692 │
│ 2017-2019 │ その他            │   260 │
│ 2017-2019 │ スライド/動画     │   170 │
│ 2017-2019 │ チュートリアル    │    93 │
│ 2017-2019 │ ライブラリ/ツール │   446 │
│ 2017-2019 │ リリースノート    │   862 │
│ 2017-2019 │ 書籍関連          │   135 │
│ 2017-2019 │ 解説記事          │   813 │
│ 2020-2022 │ その他            │   194 │
│ 2020-2022 │ スライド/動画     │    60 │
│ 2020-2022 │ チュートリアル    │    33 │
│ 2020-2022 │ ライブラリ/ツール │   397 │
│ 2020-2022 │ リリースノート    │   937 │
│ 2020-2022 │ 書籍関連          │    73 │
│ 2020-2022 │ 解説記事          │   617 │
│ 2023-2025 │ その他            │   116 │
│ 2023-2025 │ スライド/動画     │    49 │
│ 2023-2025 │ チュートリアル    │    14 │
│ 2023-2025 │ ライブラリ/ツール │   191 │
│ 2023-2025 │ リリースノート    │   986 │
│ 2023-2025 │ 書籍関連          │    45 │
│ 2023-2025 │ 解説記事          │   526 │
└───────────┴───────────────────┴───────┘
```
...
| 年 | はてなダイアリー | はてなブログ | Qiita | Zenn | Medium(@) | dev.to |
|----|------------------|--------------|-------|------|-----------|--------|
| 2011 | 90 | 2 | 0 | 0 | 0 | 0 |
| 2013 | 18 | 27 | 5 | 0 | 0 | 0 |
| 2016 | 0 | 10 | 45 | 0 | 27 | 0 |
...
SELECT
    YEAR(CAST(date AS TIMESTAMP)) as year,
    SUM(CASE WHEN url LIKE '%d.hatena.ne.jp%' THEN 1 ELSE 0 END) as hatena_d,
    SUM(CASE WHEN url LIKE '%hatenablog.%' THEN 1 ELSE 0 END) as hatenablog,
    SUM(CASE WHEN url LIKE '%qiita.com%' THEN 1 ELSE 0 END) as qiita,
    SUM(CASE WHEN url LIKE '%zenn.dev%' THEN 1 ELSE 0 END) as zenn,
    SUM(CASE WHEN url LIKE '%medium.com/@%' THEN 1 ELSE 0 END) as medium_at,
    SUM(CASE WHEN url LIKE '%dev.to%' THEN 1 ELSE 0 END) as dev_to
FROM read_json_auto('https://jser.info/source-data/items.json', ignore_errors=true)
...
```
┌───────┬──────────┬────────────┬────────┬────────┬───────────┬────────┐
│ year  │ hatena_d │ hatenablog │ qiita  │  zenn  │ medium_at │ dev_to │
├───────┼──────────┼────────────┼────────┼────────┼───────────┼────────┤
│  2011 │       90 │          2 │      0 │      0 │         0 │      0 │
│  2012 │       46 │         22 │      3 │      0 │         0 │      0 │
│  2013 │       18 │         27 │      5 │      0 │         0 │      0 │
